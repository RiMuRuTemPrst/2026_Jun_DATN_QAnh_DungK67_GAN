...
A residual connection adds the input of a block directly to its output through a skip path, so that the convolutional path only has to learn a residual mapping $F(x)$ instead of a full transformation:
\begin{equation}
y = x + F(x),
\end{equation}
where $x$ is the input feature map and $F(x)$ is the transformation produced by the convolutional path. Residual connections improve gradient flow and feature reuse, which makes deep networks easier to train. The building block that implements this pattern is called a residual block (ResBlock). In the HiFiC generator, the ResBlock combines convolution, normalization, activation, and a skip connection, and is repeated many times. Because these repeated ResBlocks dominate the inference computation, they are the main target for hardware acceleration in this thesis.
