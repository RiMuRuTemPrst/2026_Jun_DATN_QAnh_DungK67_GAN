...
\begin{table}[H]
    \centering
    \caption{Hardware resource utilization and power of the full HiFiC generator accelerator on the ZCU104}
    ...
    \begin{tabular}{|l|c|}
        ...
        \hline
        Total on-chip power (W) & $\approx$10.2 \\
        \hline
    \end{tabular}
\end{table}
...
On-board (silicon) power was not measured in this work; instead, the design power was estimated with the Vivado power analysis tool after synthesis, and a direct measurement on the physical board is left for future deployment. For the synthesized full generator constrained at 300~MHz, Vivado reports a total on-chip power of approximately 10.2~W, comprising about 9.6~W of dynamic power and 0.7~W of static device leakage. Consistent with the compute-bound, DSP-heavy nature of the design, the dynamic power is dominated by the DSP arithmetic ($\approx$3.0~W), the CLB logic ($\approx$2.6~W), and signal switching ($\approx$1.8~W), while the BRAM and URAM buffers together account for about 1.1~W; this confirms that reducing unnecessary memory movement and improving data reuse benefit power as well as performance. The estimate is vectorless (default switching activity, medium confidence) and therefore approximate, but it places the programmable-logic power at roughly 10~W---about $8\times$ below the 79.65~W measured for the same generator on a desktop-class GPU. This low absolute power envelope, suited to embedded and thermally constrained deployment rather than to winning raw wall-clock latency, is the angle developed in the comparison below.
